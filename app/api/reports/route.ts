...
export async function POST(req: NextRequest) {
  ...
  const result = db.prepare(`
    ...
    VALUES (?, ?, ?, ?)
  `).run(session.id, date, finalTitle, JSON.stringify(data)) as any;
  ...
  if (envEmail) {
    ...
    const managers = db.prepare("SELECT email FROM users WHERE role = 'finance_manager' AND is_active = 1").all() as { email: string }[];
  ...
  return NextResponse.json({ success: true, id: result.lastInsertRowid, data, emailSent, emailReason, sentTo: recipients });
}