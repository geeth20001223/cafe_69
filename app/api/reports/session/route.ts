...
export async function POST(req: NextRequest) {
  ...
  const salesRes = await db.execute({
    sql: `
      ...
      WHERE session_type = ? AND DATE(datetime(created_at, '+5 hours', '30 minutes', '-7 hours')) = ?
    `,
  ...
  const itemsRes = await db.execute({
    sql: `
      ...
      WHERE s.session_type = ? AND DATE(datetime(s.created_at, '+5 hours', '30 minutes', '-7 hours')) = ?
